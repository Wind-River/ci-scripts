...
docker-compose exec -T layerindex /bin/bash -c "cd /opt/layerindex; python3 manage.py layer_update ${ARGS[*]}"

docker-compose exec -T layerindex /bin/bash -c "cd /opt/layerindex/layerindex; ./update.py --branch=$DEVBUILD_BRANCH -l $DEVBUILD_LAYER_NAME --fullreload"
